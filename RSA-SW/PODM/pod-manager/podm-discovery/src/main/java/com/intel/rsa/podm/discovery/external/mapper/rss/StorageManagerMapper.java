...
@Dependent
public class StorageManagerMapper extends DomainObjectMapper<RssStorageManager, Manager> {
    @Inject
    private DomainObjectRepository repository;

    public StorageManagerMapper() {
        super(RssStorageManager.class, Manager.class);
    ...
    @Override
    protected void performNotAutomatedMapping(RssStorageManager source, Manager target) {
        super.performNotAutomatedMapping(source, target);
        target.updateModified();
        target.setStorageManager(true); // TODO workaround
    }
}
