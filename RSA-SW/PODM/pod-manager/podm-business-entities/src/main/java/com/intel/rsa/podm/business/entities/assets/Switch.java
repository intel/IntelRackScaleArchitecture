...
public class Switch extends ManagedAsset {
    ...
    public FabricModule getFabricModule() {
        try {
            return single(getLinked(AssetLink.CONTAINED_BY, FabricModule.class));
        } catch (IllegalStateException e) {
            throw new IllegalStateException("Switch has to be contained by exactly one Fabric Module", e);
        }
    }

    public Collection<SwitchPort> getSwitchPorts() {
        return getLinked(AssetLink.CONTAINS, SwitchPort.class);
    }
}
