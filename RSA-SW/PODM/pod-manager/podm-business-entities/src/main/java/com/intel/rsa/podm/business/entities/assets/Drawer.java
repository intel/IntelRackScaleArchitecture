...
public class Drawer extends ManagedAsset {
    ...
    public Rack getRack() {
        try {
            return single(getLinked(AssetLink.CONTAINED_BY, Rack.class));
        } catch (IllegalStateException e) {
            throw new IllegalStateException("Drawer has to be contained by exactly one Rack", e);
        }
    }

    public Collection<ComputeModule> getComputeModules() {
        return getLinked(AssetLink.CONTAINS, ComputeModule.class);
    }

    public ComputeModule getComputeModule(Id id) {
        return DomainObjectHelper.findById(id, getComputeModules());
    }

    public Collection<FabricModule> getFabricModules() {
        return getLinked(AssetLink.CONTAINS, FabricModule.class);
    }

    public FabricModule getFabricModule(Id id) {
        return DomainObjectHelper.findById(id, getFabricModules());
    }
