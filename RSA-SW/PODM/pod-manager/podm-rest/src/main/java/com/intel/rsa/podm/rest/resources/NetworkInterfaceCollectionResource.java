...
import com.intel.rsa.podm.business.services.NetworkInterfaceService;
...
public class NetworkInterfaceCollectionResource extends BaseResource {
    ...
    public CollectionDto get() {
        ...
        try {
            //todo move this to separate resources (for example to Manager and Blade)
            if (ContextType.MANAGER.equals(context.getType())) {
                return service.getNetworkInterfaceCollectionForManager(context);
            } else {
                return service.getNetworkInterfaceCollectionForBlade(context);
            }
