...
function(install_openipmi)
    include(ConfigurationPackage OPTIONAL)

    set(OPENIPMI_SOURCE_PACKAGE
        "http://sourceforge.net/projects/openipmi/files/OpenIPMI%202.0%20Library/2.0.18/OpenIPMI-2.0.18.tar.gz"
    )
    ...
    if (openipmi_library AND openipmi_include)
        return()
    endif()

    set(download_dir ${CMAKE_CURRENT_LIST_DIR}/../third_party)
    set(source_package ${OPENIPMI_SOURCE_PACKAGE})
    ...
    if (EXISTS ${source_dir})
        ...
        list(APPEND CONFIGURE_FLAGS --with-openssl=yes)
        list(APPEND CONFIGURE_FLAGS --with-glib=no)
        list(APPEND CONFIGURE_FLAGS --with-glib12=no)
        list(APPEND CONFIGURE_FLAGS --with-tcl=no)
        list(APPEND CONFIGURE_FLAGS --with-perl=no)
        list(APPEND CONFIGURE_FLAGS --with-python=no)
        list(APPEND CONFIGURE_FLAGS --with-tkinter=no)
        list(APPEND CONFIGURE_FLAGS --with-swig=no)
        list(APPEND CONFIGURE_FLAGS LDFLAGS=-L${CMAKE_BINARY_DIR}/lib)
        list(APPEND CONFIGURE_FLAGS
            CPPFLAGS=-I${CMAKE_BINARY_DIR}/include\ -I${CMAKE_BINARY_DIR}/include/ncurses
        )
