...
void RegisterAgent::method(const Json::Value& params, Json::Value& result) {
    try {
        ...
        for (const auto& param : params["capabilities"]) {
            request.m_capabilities.emplace_back(
                                        param["name"].asString(),
                                        param["version"].asString());
        }

        command->execute(request, response);

        result["ipv4address"] = response.m_ipv4address;
        result["port"] = response.m_port;
        result["version"] = response.m_version;
        result["gamiId"] = response.m_gami_id;
