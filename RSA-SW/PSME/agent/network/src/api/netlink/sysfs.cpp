...
std::string SysFs::get_switch_port_attribute(SwPortAttribute attr) const {
    ...
    udev_enumerate_add_match_property(udev_enum.get(), "ID_NET_DRIVER", DRIVER);
    udev_enumerate_add_match_sysname(udev_enum.get(), ifname.c_str());
...
const std::vector<std::string> SysFs::get_port_list() const {
    ...
    while (nullptr != dev_entry) {
        ...
        /* add port to the list except CPU port */
        if (CPU_PORT != name) {
            port_list.push_back(std::string(name));
        }
