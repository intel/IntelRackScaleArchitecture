...
Errors::Types Response::get_error() const {
    return Errors::Types(m_response_data.m_error);
}

bool Response::is_valid() const {
    return Errors::Types::SUCCESS == get_error();
}

std::uint32_t Response::get_length() const {
    return m_response_data.m_length -
                    static_cast<std::uint32_t>(get_response_pod_size());
}
