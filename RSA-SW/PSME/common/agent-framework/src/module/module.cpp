...
ModuleState::State Module::get_state() const {
    return m_state_machine->get_curr_state()->get_state();
}

const std::string Module::get_state_name() const {
    return m_state_machine->get_curr_state()->get_state_name(get_state());
}
...
std::vector<HardDriveSharedPtr> Module::get_hard_drives() const {
    ...
    const auto& submodules = get_submodules();
    for (const auto& submodule : submodules) {
        auto submodule_drives = std::move(submodule->get_hard_drives());
