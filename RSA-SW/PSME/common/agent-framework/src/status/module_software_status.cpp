...
ModuleStatus::Status ModuleSoftwareStatus::read_status() {

    ModuleStatus::Status status = ModuleStatus::Status::UNKNOWN;
...
bool ModuleSoftwareStatus::send_ping_receive_pong(int sock,
    ...
    log_debug(GET_LOGGER("status"), "Dest:" << inet_ntoa(socket.sin_addr));
    ...
    for(int i = 0; i < PING_COUNT; i++) {
        if(recvfrom(sock, &packet, sizeof(packet), 0, &dest_addr, &size) > 0) {
            ...
            log_debug(GET_LOGGER("status"), "Source:" << inet_ntoa(dest_socket.sin_addr));
