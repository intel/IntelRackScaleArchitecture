...
TEST_F(ModuleTest, PositiveInitModule) {
    ...
    m_module->init();
    auto& state_machine = m_module->get_state_machine();

    state = state_machine->get_curr_state();
    ASSERT_EQ(state->get_state(), ModuleState::State::UNKNOWN);
    ASSERT_EQ(m_module->get_state(), ModuleState::State::UNKNOWN);
    ASSERT_EQ(m_module->get_state_name(), "Unknown");
    ASSERT_EQ(m_module->get_ip_address(), TEST_IP_ADDRESS);
    ASSERT_NE(m_module->get_name(), "");
    ASSERT_NE(m_module->get_hw_status(), nullptr);
    ASSERT_NE(m_module->get_sw_status(), nullptr);

    // Stops state machine, so actually, does nothing.
    m_module->clean();
}
