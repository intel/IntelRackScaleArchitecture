...
#include "json/get_drive_info.hpp"
...
public:
    ...
    void execute(const Request& request, Response& response) {
        ...
        if (uuid != m_component || drive != m_drive || controller != m_controller) {
...
TEST_F(GetDriveInfoTest, PositiveExecute) {
    ...
    GetDriveInfo* command = new GetDriveInfo("TestModuleId", 1, 1);
    ...
    ASSERT_TRUE(result["capacityGB"].isUInt());
    ASSERT_TRUE(result["rpm"].isUInt());
...
TEST_F(GetDriveInfoTest, NegativeDriveNotFound) {
    compute::json::GetDriveInfo command_json;
    GetDriveInfo* command = new GetDriveInfo("TestModuleId", 1, 1);
