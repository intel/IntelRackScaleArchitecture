...
int main(int argc, char** argv) {
    ...
    auto password_length = strlen(argv[1]);
    if (password_length > CFG_PASS_LENGHT_MAX-1) {
        cerr << "error: Password too long"  << endl;
        cout << "Password length: " << password_length
             << "\nPassword max length: " << CFG_PASS_LENGHT_MAX << endl;
...
void encrypt_password(const char* password,
    ...
    for (uint32_t i = 0; i < blocks_number; i++) {
		...
		for (uint32_t j = 0; j < CFG_OCTET; j++) {
			uint8_t c = 0;
			for (uint32_t k = 0; k < CFG_OCTET; k++) {
				c |= uint8_t(password_bits[j * CFG_OCTET + k] << k);
			}
