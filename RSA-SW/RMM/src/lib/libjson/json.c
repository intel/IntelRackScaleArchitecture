 ...
 *
 * Modified by Intel Corporation for ASCII (NOT UTF-8) JSON parse.
 */
...
static int do_format(json_t *json, strbuff_t *strbuf)
{
	...
	case JSON_INTEGER:
	{
		...
		size = snprintf(data, sizeof(data), "%lld", json_integer_value(json));
		if (size >= sizeof(data))
			return 0;
	...
	case JSON_REAL:
	{
		...
		size = snprintf(data, sizeof(data), "%.16g", json_real_value(json));
		if (size >= sizeof(data))
			return 0;
...
int add_json_string(json_t *result, const char *name, const unsigned char *input)
{
	json_t *rs_json = NULL;

	rs_json = json_string((char *)input);
	if (rs_json == NULL) {
		json_free(result);
		return -1;
	}
