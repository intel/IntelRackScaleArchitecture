...
char *insert_str(char *dest, const char *source, unsigned int first, const char *ins_str)
{
	...
	(first < 0) || (first > strlen(source))) {
		return NULL;
	}

	len_ins_str = strlen(ins_str);
	memset(dest, 0, (strlen(source) + len_ins_str));

	if (first > 0)
		memcpy(dest, source, first);

	memcpy(dest + first, ins_str, len_ins_str);
	memcpy(dest + first + len_ins_str, source + first, (strlen(source) - first));
...
int hexstr2buf(const char *hex_str, int size, char *buf)
{
	if ((hex_str == NULL) || (buf == NULL))
		return -1;
	else if ((size <= 0) || (strlen(hex_str) != size))
		return -2;
	else if ((strlen(hex_str))%2 != 0)
		return -3;
	else {
		int i = 0;
