...
int create_udp_listen(unsigned int host, int port, int broadcast, int reuse)
{
	...
	if (fcntl(fd, F_SETFL, val | O_NONBLOCK) < 0) {
		rmm_log(ERROR, "Failed to set to nonblock: %s\n", strerror(errno));
		goto err;
	}
	...
	if (broadcast) {
		...
		if (rc < 0) {
			rmm_log(ERROR, "%s", "setsockopt failed");
			goto err;
		}
	}
	if (reuse) {
		...
		if (setsockopt(fd, SOL_SOCKET, SO_REUSEADDR, (void*)&val, sizeof(val)) < 0) {
			rmm_log(ERROR, "%s", "setsockopt SO_REUSEADDR");
			goto err;
		}
	...
	if (rc < 0) {
		rmm_log(ERROR, "Failed to bind: %s\n", strerror(errno));
		goto err;
	}
...
int udp_connect(unsigned int host, int port)
{
	...
	if (rc < 0) {
		rmm_log(ERROR, "connect fail: %s\n", strerror(errno));
		goto err;
	}

	val = fcntl(fd, F_GETFL, 0);
	if (fcntl(fd, F_SETFL, val | O_NONBLOCK) < 0) {
		rmm_log(ERROR, "set nonblock fail: %s\n", strerror(errno));
		goto err;
	}
	fcntl(fd, F_SETFD, FD_CLOEXEC);
