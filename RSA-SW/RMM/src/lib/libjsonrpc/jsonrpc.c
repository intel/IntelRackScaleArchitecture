...
json_t * jrpc_init_pkg()
{
	...
	if (NULL != req) {
		ver = json_string("2.0");
		if (NULL != ver) {
			if (json_object_add(req, "json_rpc", ver) != JSON_SUCCESS)	
				return NULL;
			else 
				return req;
		}
...
{
	char * ver_str = NULL;
	json_t * ver = json_object_get(object, "json_rpc");
...
char * jrpc_do_create_req_string(jrpc_req_type_t type, int64 id, char * method, uint8 num_of_params, jrpc_param_t * param_t)
{
	...
	if (NULL == (req = jrpc_do_create_req(type, id, method, num_of_params, param_t)) ||
		NULL == (string = malloc(JSONRPC_MAX_STRING_LEN)) ||
		jrpc_format_string(req, string, JSONRPC_MAX_STRING_LEN))
		return NULL;
