...
int connect_registerd()
{
	int port = rmm_cfg_get_port(REGISTERD_PORT); 
	if(port == 0) {
		rmm_log(ERROR, "Can't get registerd port...\n");
		return -1;
	}
	fd = udp_connect(INADDR_LOOPBACK, port);
	if (fd < 0) {
		rmm_log(ERROR, "Connect registerd failed...\n");
		return -1;
	}
...
int send_msg_to_registerd(jrpc_req_pkg_t *req, int evt_id)
{
	int rc = -1;
	jrpc_rsp_pkg_t rsp = {};
