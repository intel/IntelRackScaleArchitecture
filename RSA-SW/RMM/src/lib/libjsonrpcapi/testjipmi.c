...
static int dev_id_resp_handler_br(int result, unsigned char *rsp, int rsp_len,
			void *cb_data)
{
	int i;
...
void dual_br_msg_req_via_lan(unsigned int host, char *username, char *passwd)
{
	...
	libjipmi_rmcp_cmd_send_msg(host, IPMI_RMCP_PORT, &req, dev_id_resp_handler_br, NULL,
							0x10,
							0x2,
							0x24,
							0x0,
...
static void ipmi_br_req_test(void)
{
	...
	libjipmi_rmcp_cmd_send_msg(host, IPMI_RMCP_PORT, &req, dev_id_resp_handler_br, NULL ,0x20,0x03, JIPMI_SYNC);
...
int main(int argc, char **argv)
{
	...
	username = argv[1];
	passwd = argv[2];
	global_cm_addr = inet_addr("127.0.0.1");

	request_dev_id(global_cm_addr, username, passwd);
//	br_msg_req_via_lan(global_cm_addr);
//	dual_br_msg_req_via_lan(global_cm_addr);
