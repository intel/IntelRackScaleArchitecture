...
static int handle_jipmi_msg(char *msg)
{
	...
	if (result == NULL) {
		...
		memcpy(rsp_data, &err_code, sizeof(int64));
		ret = -1;
		...
		rsp_data = malloc(strlen(origin_data));
		if (rsp_data == NULL) {
			IPMI_DEBUG("jipmi: fail to alloc buffer\n");
			goto end;
		}
        	data_len = hexstr2buf(origin_data, strlen(origin_data), (char *)rsp_data);
		/*if (rsp_data == NULL) {
...
static int send_jmsg_to_ipmid(char *msg, int msg_id, unsigned int timeout, sync_mode_t mode, bool bridge_level)
{
	...
	if(mode == JIPMI_NON_SYNC) {
		...
		sendto(sync_jipmi_fd, msg, IPMI_JSONRPC_MAX_MSG_LEN, 0, (struct sockaddr *)&dest, sizeof(dest));
		...
		for (;;) {
			...
			rc = recv(sync_jipmi_fd, rsp_string, sizeof(rsp_string), 0);
...
static int jipmi_ipmb_cmd_timeout(unsigned char sa, jipmi_msg_t *req,
			jipmi_rsp_callback_fn cb_fn, void *cb_arg, unsigned int timeo, sync_mode_t mode)
{
	int i = 0;
	int64 jrpc_id = 0;
	...
	jrpc_param_t param[MAX_RPC_LEN] = {{0}};
	char buffer[IPMI_JSONRPC_MAX_MSG_LEN] = {0};
	char str_ipmb_src[20] = {0};

	jrpc_id = join_handle_list(cb_fn, cb_arg, timeo, is_ipmb_addr_bcast(sa));

	param[i].name = STR_IPMB_SRC_ADDR;
	FILL_INT(str_ipmb_src, sa);
	param[i].value = (void *)&(str_ipmb_src);
	param[i].value_type = JSON_STRING;
	i++;

	jipmi_set_base_info(param, &i, req);

	jreq = jrpc_create_req(jrpc_id, JSON_RPC_IPMB_REQ, i, param);
	jrpc_format_string(jreq, buffer, sizeof(buffer));
	if (jreq)
		json_free(jreq);
	return send_jmsg_to_ipmid(buffer, jrpc_id, timeo, mode, false);
...
int libjipmi_ipmb_cmd(unsigned char sa, jipmi_msg_t *req,
			jipmi_rsp_callback_fn cb_fn, void *cb_arg, sync_mode_t mode)
{
	return jipmi_ipmb_cmd_timeout(sa, req, cb_fn, cb_arg,
				IPMI_DFLT_TIMEOUT_MS, mode);
}


int libjipmi_ipmb_cmd_broadcast(jipmi_msg_t *req,
			jipmi_rsp_callback_fn cb_fn, void *cb_arg, sync_mode_t mode)
{
	return jipmi_ipmb_cmd_timeout(IPMI_IPMB_BCAST_ADDR, req, cb_fn, cb_arg,
				IPMI_DFLT_TIMEOUT_MS, mode);
}
...
int libjipmi_rmcp_close_session(unsigned int host, unsigned int port, jipmi_rsp_callback_fn cb_fn,
								void *cb_arg, sync_mode_t mode)
{
	jipmi_msg_t req = { {0} };

	snprintf(req.netfn, MAX_NUM_LEN, "%d", IPMI_RMCP_CLOSE_SESSION_NETFN);
	snprintf(req.cmd, MAX_NUM_LEN, "%d", IPMI_RMCP_CLOSE_SESSION_CMD);

	/* Notes: the data for closing session will be filled by ipmid */
	snprintf(req.data_len, MAX_NUM_LEN, "%d", 0);
			...
			unsigned long transit_addr, unsigned short transit_channel, int bridge_level, sync_mode_t mode)
{
	...
	memcpy(&(req->msg_base), request, sizeof(jipmi_msg_t));
