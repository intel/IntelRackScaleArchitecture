...
int assetd_psu_power_in_change(int64 *pin, char * uuid)
{
	jrpc_req_pkg_t req_pkg = {};
	int64 type = PSU_TYPE;
...
int assetd_psu_current_out_change(int64 * iout, char * uuid)
{
	jrpc_req_pkg_t req_pkg = {};
	int64 type = PSU_TYPE;
...
int assetd_drawer_ip_change(char *ip, char *uuid)
{
	jrpc_req_pkg_t req_pkg = {};
	int64 type = DRAWER_TYPE;

	fill_pkg_hdr(&req_pkg, &type, uuid, NULL);
	fill_param(&req_pkg, JRPC_IPADDRESS, ip, JSON_STRING);
