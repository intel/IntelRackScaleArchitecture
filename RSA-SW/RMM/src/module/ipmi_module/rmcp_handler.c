...
static void process_rmcp_plus_msg(unsigned char *rmcp_msg, int rmcp_len, unsigned int host, unsigned int port)
{
	...
	if ((ipmi_info.payload_type & IPMI20_PAYLOAD_TYPE_MASK) ==
		...
		memcpy(&ipmi_info.oem_iana, &msg[offset], IPMI20_OEM_IANA_LEN);
		offset += IPMI20_OEM_IANA_LEN;

		memcpy(&ipmi_info.oem_payload_id, &msg[offset], IPMI20_OEM_PAYLOAD_ID_LEN);
		offset += IPMI20_OEM_PAYLOAD_ID_LEN;
	}

	memcpy(&ipmi_info.session_id, msg + offset, IPMI20_SESSION_ID_LEN);
	offset += IPMI20_SESSION_ID_LEN;

	memcpy(&ipmi_info.session_seq, msg + offset, IPMI20_SESSION_SEQ_NUM_LEN);
	offset += IPMI20_SESSION_SEQ_NUM_LEN;

	msg += offset;
	len -= offset;

	memcpy(&ipmi_info.ipmi_data_len, msg, IPMI20_MSG_PAYLOAD_LEN_LEN);
...
void handle_rmcp_msg(unsigned char *msg, int len, unsigned int host, unsigned int port)
{
...
#ifdef DEBUG_IPMI
	{
		...
		snprintf(__hdr, sizeof(__hdr), "RMCP IN("NIPQUAD_FMT"), port %d: ", NIPQUAD(host), ntohs(port));
		for (__i = 0; __i < len; __i++) {
			snprintf(__tmp, sizeof(__tmp), "%02X ", msg[__i]);
			strncat(__body, __tmp, 8);
		}
		snprintf(__end, sizeof(__end), "\n");
		snprintf(__buff, sizeof(__buff), "%s%s%s", __hdr, __body, __end);
	...
	if (rmcp->class == RMCP_CLASS_IPMI) {
		...
		if ((ipmi_info.auth_type & IPMI_AUTH_TYPE_MASK) != IPMI20_AUTH_TYPE_RMCP_PLUS) {
			...
			memcpy(&ipmi_info.session_id, &msg[IPMI_SESSION_ID_OFFSET], IPMI_SESSION_ID_LEN);
