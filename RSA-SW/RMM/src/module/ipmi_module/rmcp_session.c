			...
			ipmi_json_ipc_header_t *header)
{
	...
	*user_port = req->user_port;
	...
	if (strlen((const char *)(req->header.method)) < JRPC_METHOD_MAX)
		memcpy(header->method, req->header.method, strlen((const char *)(req->header.method)));
			...
			ipmi_json_ipc_header_t header)
{
	...
	if (strlen((const char *)header.method) < JRPC_METHOD_MAX)
		memcpy(req->header.method, header.method, strlen((const char *)header.method));
			...
			struct ipmi_msg *req, unsigned char seq)
{
	...
	memcpy(&ipmi[IPMI_SESSION_ID_OFFSET], &sess->session_id, IPMI_SESSION_ID_LEN);
	...
	if (req->data_len > 0) {
		memcpy(&ipmi[IPMI_DATA_OFFSET], req->data, req->data_len);
		len += req->data_len;
	}
			...
			struct ipmi_msg *req, unsigned char seq, int bridge_level)
{
	...
	if (sess->state >= IPMI_SESS_STATE_SESSION_ACTIVATE_SENT && sess->auth_type) {
		memcpy(msg+len, sess->password, 16);
		len += 16;
	}
	...
	/* message data */
	if (req->data_len) {
		memcpy(msg+len, req->data, req->data_len);
		len += req->data_len;
	}
...
static void send_activate_session_cmd(struct ipmi_session *sess)
{
	...
	memcpy(&ipmi.data[2], sess->challenge, 16);
			...
			struct rmcp_ipmi_info *ipmi_info)
{
	...
	memcpy(&sess->session_id, &ipmi_info->ipmi_data[2], 4);
...
static void send_session_challenge_cmd(struct ipmi_session *sess)
{
	...
	ipmi.data_len = 17;
	ipmi.data[0] = sess->auth_type;
	memcpy(&ipmi.data[1], sess->username, 16);
			...
			struct rmcp_ipmi_info *ipmi_info)
{
	...
	memcpy(&sess->session_id, &ipmi_info->ipmi_data[1], 4);
	memcpy(sess->challenge,   &ipmi_info->ipmi_data[5], 16);
												...
												int *msglen)
{
	...
	memcpy(buf + buf_len, &(ipmi_info->session_id), IPMI20_SESSION_ID_LEN);
	buf_len += IPMI20_SESSION_ID_LEN;

	memcpy(buf + buf_len, &(ipmi_info->session_seq), IPMI20_SESSION_SEQ_NUM_LEN);
	...
	if ((sess->state > IPMI20_SESS_STATE_RAKP_3_SENT)
		...
		if (req->data_len > 0) {
			memcpy(data + IPMI_DATA_OFFSET, req->data, req->data_len);
		...
		memcpy(ipmi_data_len_pos, &(data_crypt_len), IPMI20_MSG_PAYLOAD_LEN_LEN);
		buf_len += IPMI20_MSG_PAYLOAD_LEN_LEN;

		/* IPMI Payload */
		if (data_crypt_len > 0) {
			memcpy(buf + buf_len, data_crypt, data_crypt_len);
		...
		if ((sess->integrity_alg > 0)) {
			...
			memcpy(buf + buf_len, auth_code, 12);
			buf_len += 12;
		}
	} else {
		memcpy(buf + buf_len, &(ipmi_info->ipmi_data_len), IPMI20_MSG_PAYLOAD_LEN_LEN);
		buf_len += IPMI20_MSG_PAYLOAD_LEN_LEN;

		memcpy(buf + buf_len, ipmi_info->ipmi_data, ipmi_info->ipmi_data_len);
...
static void send_session_open_session_cmd(struct ipmi_session *sess)
{
	...
	memcpy(msg + offset, &(sess->session_id), IPMI20_OPEN_SESS_REQ_SESSION_ID_LEN);
														 ...
														 struct rmcp_ipmi_info *ipmi_info)
{
	...
	if ((ipmi_info->data_len > 3) && (ipmi_info->data_len < 18)) {
		memcpy(cipher_suite_data + cipher_suite_data_len, ipmi_info->ipmi_data + 2, ipmi_info->data_len - 2);
		...
		memcpy(cipher_suite_data + cipher_suite_data_len, ipmi_info->ipmi_data + 2, 16);
...
static void send_session_rakp_msg_1_cmd(struct ipmi_session *sess)
{
	...
	/* BMC session ID */
	memcpy(msg + offset, &(sess->payload.open_sess_resp.bmc_id),
	...
	memcpy(msg + offset, sess->session_random,
	...
	user_name_len = strlen((char *)sess->username);
	msg[offset++] = user_name_len;
	memcpy(msg + offset, sess->username, user_name_len);
									 ...
									 struct rmcp_plus_ipmi_info *ipmi_info)
{
	...
	memcpy(&(sess->payload.open_sess_resp.session_id),
	...
	memcpy(&(sess->payload.open_sess_resp.bmc_id),
...
static void send_session_rakp_msg_3_cmd(struct ipmi_session *sess)
{
	...
	/* RMCP+: BMC Session ID*/
	memcpy(msg + offset, &(sess->bmc_id), IPMI20_RAKP_MSG_3_BMC_ID_LEN);
	...
	if (sess->payload.rakp_msg_2.stat_code == RMCP_PLUS_STAT_NO_ERR) {
		...
		memcpy(data_check, sess->bmc_random, IPMI20_RAKP_BMC_RANDOM_LEN);
		len_check += IPMI20_RAKP_BMC_RANDOM_LEN;
		memcpy(data_check + len_check, &(sess->session_id), IPMI20_SESSION_ID_LEN);
		len_check += IPMI20_SESSION_ID_LEN;
		memcpy(data_check + len_check, &(sess->privilege), IPMI20_PRIVILEGE_LEN);
		len_check += IPMI20_PRIVILEGE_LEN;

		user_name_len = strlen((char *)sess->username);
		memcpy(data_check + len_check, &user_name_len, 1);
		len_check += 1;

		memcpy(data_check + len_check, sess->username, user_name_len);
		len_check += user_name_len;

		auth_code_len = ipmi20_generate_auth_code(sess->auth_alg, sess->password,
												  strlen((char *)sess->password),
								  ...
								  struct rmcp_plus_ipmi_info *ipmi_info)
{
	...
	case AUTH_ALG_RAKP_HMAC_SHA1:
		memcpy(sess->payload.rakp_msg_2.auth_alg,
								  ...
								  struct rmcp_plus_ipmi_info *ipmi_info)
{
	...
	memcpy(&(sess->payload.rakp_msg_4.session_id),
	...
	case AUTH_ALG_RAKP_NONE:
		...
		break;
	case AUTH_ALG_RAKP_HMAC_SHA1:
		memcpy(sess->payload.rakp_msg_4.int_check,
	...
	memcpy(data_verify, sess->session_random, IPMI20_RAKP_SESSION_RANDOM_LEN);
	data_verify_len = IPMI20_RAKP_SESSION_RANDOM_LEN;
	memcpy(data_verify + data_verify_len,
	...
	data_verify[data_verify_len++] = strlen((char *)sess->username);
	memcpy(data_verify + data_verify_len, sess->username, strlen((char *)sess->username));
	data_verify_len += strlen((char *)sess->username);
	if (sess->key_kg[0] != '\0') {
		memcpy(key_auth, sess->key_kg, 20);
	} else {
		memcpy(key_auth, sess->password, strlen((char *)sess->password));
	}
...
static void flush_ipmi20_user_msg(struct ipmi_session *sess)
{
	...
	list_for_each_entry_safe(msg, msg_next, &sess->msg_list, list) {
		...
		match.netfn = msg->msg.netfn;
		match.cmd   = msg->msg.cmd;
		if (rmcp_intf_next_seq(sess, &match, msg->timeo, msg->msgid, msg->user_port) != 0)
			continue;
								   ...
								   struct rmcp_plus_ipmi_info *ipmi_info)
{
	...
	if ((ipmi->msg.data_len > 0) && (ipmi->msg.data_len <= IPMI_MAX_DATA_LENGTH))
		memcpy(ipmi->msg.data, payload + IPMI_DATA_OFFSET, data_len);
			...
			unsigned short user_port, ipmi_json_ipc_header_t header)
{
	...
	if (sess->state != IPMI_SESS_STATE_ACTIVE) {
		...
		if (strlen((const char *)header.method) < JRPC_METHOD_MAX)
			memcpy(msgsent->header.method, header.method, strlen((const char *)header.method));

		memcpy(&msgsent->msg, ipmi, offset_of(struct ipmi_msg, data) + ipmi->data_len);
		...
		if (sess->auth_type == IPMI20_AUTH_TYPE_RMCP_PLUS) {
			...
			if (IPMI_BRIDGE_MSG_NONE != ipmi->union_app_req.bridge.bridge_level) {
				...
				if ((ipmi->netfn == IPMI_RMCP_CLOSE_SESSION_NETFN)
					...
					&& (ipmi->data_len == 0)) {
					memcpy(ipmi->data, &sess->session_id, IPMI_SESSION_ID_LEN);
			...
			struct rmcp_ipmi_info *ipmi_info)
{
	...
	if ((ipmi->msg.data_len > 0) && (ipmi->msg.data_len <= IPMI_MAX_DATA_LENGTH))
		memcpy(ipmi->msg.data, ipmi_info->ipmi_data, ipmi_info->data_len);
