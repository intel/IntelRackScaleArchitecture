...
{
	memdb_integer node_id = libdb_create_node(DB_RMM, MC_TYPE_RMC, MC_TYPE_SSA, SNAPSHOT_NEED, LOCK_ID_NULL);
...
static void load_ssa_dest_url(memdb_integer nid) 
{
	...
	for (i = 0; i < dest_count; i++) {
		...
		snprintf(str_snmp_session, sizeof(str_snmp_session), "%p", snmp_seesion);
...
void ssa_memdb_add_dest_url(char *dest, char *port)
{
	...
	snprintf(new_dest_url, sizeof(new_dest_url), "%s:%s", dest, port);
...
int ssa_memdb_remove_dest_url(char *dest, char *port)
{
    ...
    char new_dest_url[DEST_URL_LEN] = {0};
    snprintf(new_dest_url, DEST_URL_LEN, "%s:%s", dest, port);
