...
{
	if (type > MC_TYPE_END || type < MC_TYPE_RMC)
		snprintf(type_str, len, "%s", "UNKNOWN");
	else
		snprintf(type_str, len, "%s", mc_type_str[type]);
}
...
static void print_node(memdb_integer db_name, struct node *root)
{
	...
	*--indent = '\0';
	...
	if (root->snapshot_flag == SNAPSHOT_NEED)
		snprintf(tmp, sizeof(tmp), "%s", "snap_y");
	else
		snprintf(tmp, sizeof(tmp), "%s", "snap_n");
	...
	*--indent = '-';
	...
	list_for_each_entry(pa, &root->attrs, group) {
		...
		snprintf(out_buff, sizeof(out_buff), "%s-A(%s)::%s:", indent, tmp, pa->name);
		printf("%s", out_buff);
		fprintf(file_out, "%s", out_buff);

		if (pa->type == TYPE_STRING) {
			if (strstr((char *)pa->data, HARD_CODE_FLAG)) {
				offset = (unsigned char *)pa->data + (sizeof(HARD_CODE_FLAG)-1);
				snprintf(out_buff, sizeof(out_buff), "%s%s%s", COLOR_RED, offset, COLOR_NONE);
				printf("%s", out_buff);
				fprintf(file_out, "%s", out_buff);
			} else if (strstr(pa->name, "ip_addr")) {
			...
			/* dump present for 8 bit*/
				...
				for (i = 0; i < 8; i++) {
					...
					strcat(tmp, present);
					snprintf(present, sizeof(present), "%s", tmp);
				}
				printf("%s", present);
...
static void delete_node(memdb_integer db_name, struct node *root)
{
	struct node *p, *n;
...
void destroy_node(memdb_integer db_name, struct node *root)
{
	if (DB_RMM == db_name)
		freeing_node = root;
	else if (DB_POD == db_name)
		freeing_pod_node = root;
	else
...
void dump_all_subscribes(memdb_integer db_name)
{
	...
	list_for_each_entry(sub, which_sub, list) {
		if (sub->event == EVENT_NODE_CREATE) {
			snprintf(buff, sizeof(buff), "subscribe: NODE CREATE, type is %lld\n", sub->data.node.type_min);
			printf("%s", buff);
			fprintf(file_out, "%s", buff);
		} else if (sub->event == EVENT_NODE_DELETE) {
			snprintf(buff, sizeof(buff), "subscribe: NODE DELETE, type is %lld\n", sub->data.node.type_min);
			printf("%s", buff);
			fprintf(file_out, "%s", buff);
		} else if (sub->event == EVENT_NODE_ATTR) {
			snprintf(buff, sizeof(buff), "subscribe: NODE ATTR, node_id is %lld\n", sub->data.attr.node_id);
			printf("%s", buff);
			fprintf(file_out, "%s", buff);
			if (sub->data.attr.prefix_len != 0) {
				snprintf(buff, sizeof(buff), "|-----------prefix is %s\n", sub->data.attr.name_prefix);
				printf("%s", buff);
				fprintf(file_out, "%s", buff);
			}
		} else {
...
int set_node_attr(memdb_integer db_name, struct node *node, memdb_integer cookie,
				  ...
				  memdb_integer snapshot_flag, memdb_integer type)
{
	struct node_attr *pa;
	...
	if (pa == NULL) {
		...
		pa->type = type;
		memcpy(pa->name, name, namelen);
		list_add_tail(&pa->group, &node->attrs);

		if (DB_RMM == db_name) {
			list_add_tail(&pa->list, &attr_list);

			curr_attr_action = EVENT_ATTR_ACTION_ADD;
		} else if (DB_POD == db_name) {
			list_add_tail(&pa->list, &pod_attr_list);

			curr_pod_attr_action = EVENT_ATTR_ACTION_ADD;
		} else {
			free(pa);
			return -1;
		}
					 ...
					 unsigned short namelen)
{
	...
	list_for_each_entry(pa, &node->attrs, group) {
		...
		if (DB_RMM == db_name) {
			curr_attr = pa;
			curr_attr_action = EVENT_ATTR_ACTION_DEL;
		} else if (DB_POD == db_name) {
			curr_pod_attr = pa;
			curr_pod_attr_action = EVENT_ATTR_ACTION_DEL;
		} else
			return 0;
...
void publish_subscription(void)
{
	bool snap_need = false;

	if (new_node != NULL) {
		if (SNAPSHOT_NEED == new_node->snapshot_flag) {
			memdb_log(DB_RMM, MEMDB_LOG_RMM_FILE, new_node,
					  MEMDB_LOG_CREATE_NODE);
		}

		node_create_notify(DB_RMM, new_node);

		new_node = NULL;
	}

	if (new_pod_node != NULL) {
		if (SNAPSHOT_NEED == new_pod_node->snapshot_flag) {
			memdb_log(DB_POD, MEMDB_LOG_POD_FILE, new_pod_node,
					  MEMDB_LOG_CREATE_NODE);
		}

		node_create_notify(DB_POD, new_pod_node);

		new_pod_node = NULL;
	}

	if (freeing_node != NULL) {
		if (SNAPSHOT_NEED == freeing_node->snapshot_flag) {
			memdb_log(DB_RMM, MEMDB_LOG_RMM_FILE, freeing_node,
					  MEMDB_LOG_DESTROY_NODE);
		}

		delete_node(DB_RMM, freeing_node);
		freeing_node = NULL;
	}

	if (freeing_pod_node != NULL) {
		if (SNAPSHOT_NEED == freeing_pod_node->snapshot_flag) {
			memdb_log(DB_POD, MEMDB_LOG_POD_FILE, freeing_pod_node,
					  MEMDB_LOG_DESTROY_NODE);
		}

		delete_node(DB_POD, freeing_pod_node);
		freeing_pod_node = NULL;
	}

	if (curr_attr != NULL) {
		node_attr_notify(DB_RMM, curr_attr, curr_attr_action);

		if (curr_attr_action == EVENT_ATTR_ACTION_DEL) {
			if (SNAPSHOT_NEED == curr_attr->snapshot_flag) {
				memdb_log(DB_RMM, MEMDB_LOG_RMM_FILE, curr_attr,
						  MEMDB_LOG_REMOVE_ATTR);
			}
			del_node_attr(curr_attr);
		} else if (SNAPSHOT_NEED == curr_attr->snapshot_flag) {
			memdb_log(DB_RMM, MEMDB_LOG_RMM_FILE, curr_attr,
					  MEMDB_LOG_SET_ATTR);
		}
	...
	if (curr_pod_attr != NULL) {
		if (SNAPSHOT_NEED == curr_pod_attr->snapshot_flag)
			snap_need = true;

		node_attr_notify(DB_POD, curr_pod_attr, curr_pod_attr_action);

		if (curr_pod_attr_action == EVENT_ATTR_ACTION_DEL) {
			del_node_attr(curr_pod_attr);
			if (snap_need) {
				memdb_log(DB_POD, MEMDB_LOG_POD_FILE,
						  curr_pod_attr, MEMDB_LOG_REMOVE_ATTR);
			}
		} else if (snap_need) {
			memdb_log(DB_POD, MEMDB_LOG_POD_FILE,
					  curr_pod_attr, MEMDB_LOG_SET_ATTR);
		}

		curr_pod_attr = NULL;
		curr_pod_attr_action = 0;
	}
